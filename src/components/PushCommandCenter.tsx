...
export function PushCommandCenter({ currentUser }: any) {
  ...
  const handleSend = async () => {
    ...
    setIsSending(true);
    try {
      // 1. Enviar a notificaciones PWA Internas (Campanita) - Ahora soporta imágenes y links
      internalRoles.forEach(role => {
        sendNotification(role, title, message, imageUrl, destinationType, destinationValue);
      });
