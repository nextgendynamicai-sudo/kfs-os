...
export const RegisterCustomerForm = ({ onCancel, defaultReferralCode }: { onCancel: () => void, defaultReferralCode?: string }) => {
  ...
  const isFormValid = isNameValid && isPhoneValid && isPasswordValid && acceptedToS;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 animate-fade-in">
          ...
          <label className="relative h-24 rounded-xl border-2 border-dashed border-violet-200 cursor-pointer overflow-hidden flex items-center justify-center bg-violet-50/50 hover:bg-violet-100 transition-colors group block">
            ...
            {kycCedula ? (
              ...
              <div className="text-center text-violet-400 group-hover:text-violet-600 transition-colors">
                ...
                <span className="text-[10px] font-bold block mt-1 text-slate-500">Cédula de Identidad (Opcional)</span>
              </div>
            )}
          </label>
          {kycCedula && (
      ...
      <div className="relative">
        <label className="block text-xs font-black text-violet-700 uppercase tracking-widest mb-1 ml-1">Dirección (Opcional)</label>
        <div className="relative">
          <MapPin className="absolute left-4 top-4 text-violet-400" size={20} />
          <textarea placeholder="Calle, Av, Edificio, Casa (opcional)..." value={kycAddress} onChange={e => setKycAddress(e.target.value)} className="w-full bg-violet-50/50 border border-violet-100 rounded-xl pl-12 pr-4 py-3 text-violet-950 placeholder:text-slate-400 focus:outline-none focus:border-violet-400 transition-all text-sm h-20 resize-none" />
        </div>
      </div>
      ...
      <div className="flex gap-2 pt-2">
        <button type="button" onClick={onCancel} className="w-1/3 py-3 rounded-xl border border-violet-200 text-slate-500 font-bold hover:bg-violet-50 transition-all cursor-pointer">
          <span>Atrás</span>
        </button>
        <button 
          type="submit" 
