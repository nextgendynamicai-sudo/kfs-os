    ...
    <div className="min-h-screen bg-slate-50 pb-24 font-sans text-violet-950 relative">
      ...
      <div className="bg-gradient-to-br from-violet-900 to-violet-950 rounded-b-[3rem] shadow-xl shadow-violet-900/20 pt-6 pb-12 px-6 text-white relative z-10 border-b border-violet-800">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-2">
            <span className="bg-white/20 p-2 rounded-xl text-violet-300"><CheckCircle size={20} /></span>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-violet-300 font-mono text-xs mt-1 bg-violet-950/50 inline-block px-2 py-0.5 rounded-md border border-violet-800">{currentUser.phone}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 md:p-8 max-w-5xl mx-auto -mt-6 relative z-20 flex flex-col gap-8 animate-fade-in">
        ...
        {activeTab === "panel" && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white text-violet-950 p-8 rounded-[2rem] shadow-xl shadow-violet-200/50 border border-violet-100 relative overflow-hidden flex flex-col">
                <div className="relative z-10">
                  <p className="text-violet-600 text-xs font-black uppercase tracking-widest mb-2 flex items-center gap-2"><CreditCard size={14} className="text-emerald-500" /> Regalías Liquidadas (BOS)</p>
                  ...
                  {(myPromotoraData?.passiveEarningsEUR || 0) > 0 && (
                    <button onClick={() => setShowPayoutModal(true)} className="w-full bg-violet-600 text-white py-3 rounded-xl font-bold text-sm hover:scale-[1.02] transition-transform cursor-pointer shadow-lg shadow-violet-600/30 flex justify-center items-center gap-2 mt-4">
              ...
              <div className="bg-white text-violet-950 p-8 rounded-[2rem] shadow-xl shadow-violet-200/50 border border-violet-100 relative overflow-hidden flex flex-col">
                <div className="relative z-10">
                  <p className="text-violet-600 text-xs font-black uppercase tracking-widest mb-2 flex items-center gap-2"><CheckCircle size={14} className="text-violet-400" /> Nodos Captados</p>
                  <h2 className="text-5xl font-black mb-1 text-violet-950">{myPromotoraData?.setups || 0}</h2>
                  <p className="text-xs text-slate-500 mt-2">Comercios afiliados activos</p>
                </div>
                <Users size={100} className="absolute -right-10 -bottom-10 text-violet-50" />
              </div>
            </div>

            {pendingTopUps.length > 0 && (
              <div className="bg-white rounded-[2rem] shadow-xl shadow-violet-200/50 border border-violet-100 p-8 text-violet-950 mt-8 animate-fade-in">
                ...
                <div className="space-y-4">
                  {pendingTopUps.map((t: any) => {
                    ...
                    return (
                      <div key={t.id} className="bg-violet-50/50 border border-violet-100 rounded-2xl p-5 flex flex-col md:flex-row justify-between items-center gap-6">
                        <div className="flex items-center gap-4">
                          {t.screenshotBase64 && (
                            <a href={t.screenshotBase64} target="_blank" rel="noreferrer" className="w-16 h-16 rounded-xl overflow-hidden border border-violet-200 block hover:scale-105 transition-transform shadow-sm">
                              <img src={t.screenshotBase64} alt="Comprobante" className="w-full h-full object-cover" />
                            </a>
                          )}
                          <div>
                            <p className="font-bold text-sm text-violet-950">{name} <span className="text-[10px] bg-violet-200/50 px-2 py-0.5 rounded-full text-violet-700 uppercase ml-2">{t.userType}</span></p>
            ...
            <div className="bg-white rounded-[2rem] shadow-xl shadow-violet-200/50 border border-violet-100 p-8 text-violet-950 mt-8">
              ...
              <div className="space-y-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
                {(db.supportTickets || []).slice().reverse().filter((t: any) => {
                  ...
                  return (
                    <div key={ticket.id} className="bg-violet-50/50 border border-violet-100 p-4 rounded-xl flex flex-col gap-2">
                      <div className="flex justify-between items-center">
                        <p className="text-sm font-bold text-violet-950">[{ticket.status === 'open' ? '🔴 ABIERTO' : '🟢 CERRADO'}] {client?.company || "Comercio"} - {ticket.subject}</p>
                        <span className="text-[10px] text-slate-500 font-mono">{new Date(ticket.createdAt).toLocaleString()}</span>
                      </div>
                      <div className="space-y-2 mt-2 pl-4 border-l-2 border-violet-200">
                        {ticket.messages.map((m: any, i: number) => (
                          <div key={i} className="text-xs">
                            <span className="font-bold text-violet-600">{m.author}:</span> <span className="text-slate-600">{m.text}</span>
                          </div>
                        ))}
                      </div>
                      {ticket.status === 'open' && (
                        <div className="flex gap-2 mt-2">
                          <input type="text" id={`reply-promo-${ticket.id}`} placeholder="Respuesta..." className="flex-1 bg-white border border-violet-200 rounded-lg px-3 py-1.5 text-xs text-violet-950 focus:outline-none focus:border-violet-400 focus:ring-1 focus:ring-violet-400 placeholder:text-slate-400" />
                          <button onClick={() => {
                          ...
                          }} className="bg-violet-600 text-white px-3 py-1.5 rounded-lg text-xs font-black cursor-pointer hover:bg-violet-700 shadow-md shadow-violet-600/20">Responder</button>
                        </div>
                      )}
                    </div>
                  )
            ...
            {/* Manuals Section */}
            <div className="bg-gradient-to-r from-violet-950 to-violet-900 text-white p-8 md:p-10 rounded-[2rem] shadow-xl shadow-violet-900/20 relative overflow-hidden border border-violet-800">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <button onClick={() => setActiveManual('installation')} className="bg-white/10 hover:bg-white/20 p-5 rounded-2xl flex flex-col items-center justify-center gap-3 transition-colors cursor-pointer border border-white/10">
                  <DownloadCloud size={32} className="text-violet-400" />
        ...
        {activeTab === "negocios" && (
          <div className="space-y-6">
            {!showRegister ? (
              <div className="bg-white shadow-xl shadow-violet-200/50 border border-violet-100 rounded-[2rem] p-8 relative overflow-hidden">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 border-b border-violet-100 pb-4 gap-4">
                  <h3 className="text-xl font-black text-violet-950">Mis Comercios Activados</h3>
                  <div className="flex flex-wrap gap-4 w-full md:w-auto">
                    <div className="relative flex-1 md:w-48">
                      <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                      <input type="text" placeholder="Buscar comercio..." className="w-full bg-white border border-violet-200 rounded-xl pl-10 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-300 focus:border-violet-400 placeholder:text-slate-400 text-violet-950 transition-all" value={searchClient} onChange={e => setSearchClient(e.target.value)} />
                    </div>
                    <button onClick={() => setShowRegister(true)} className="bg-violet-600 text-white px-6 py-2 rounded-xl font-bold text-sm hover:bg-violet-700 transition-colors shadow-md shadow-violet-600/30 cursor-pointer">+ Nuevo Setup</button>
                  </div>
                </div>
                    ...
                    <tbody className="bg-white">
                      {filteredClients.map((c: any) => (
                        <tr key={c.id} className="border-b border-violet-100 hover:bg-violet-50/50 transition-colors">
                          <td className="py-4 px-4 font-bold text-violet-950">
                            <div className="flex items-center gap-2">
                              {c.company}
                              {c.account_tier === 'free' && (
                                <button
                                  onClick={() => upgradeToPremium(c.id, currentUser.id)}
                                  className="bg-gradient-to-r from-violet-600 to-amber-500 text-white px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider hover:opacity-90 transition-opacity whitespace-nowrap shadow-sm cursor-pointer border-none"
                                >
                                  Upgrade $5
                                </button>
                              )}
                              {c.account_tier === 'premium' && (
                                <span className="bg-violet-100 text-violet-700 border border-violet-200 px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider shadow-sm">
                                  Premium
                                </span>
                              )}
                            </div>
                          ...
                          <td className="py-4 px-4 text-center">
                            <button onClick={() => setCustomizingClient(c)} className="bg-slate-100 hover:bg-slate-200 text-violet-950 border border-slate-200 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer flex items-center gap-1 mx-auto shadow-sm">
              ...
              <div className="bg-white shadow-xl shadow-violet-200/50 border border-violet-100 rounded-[2rem] p-8 max-w-2xl mx-auto">
      ...
      {activeTab === "afiliados" && (
        <div className="space-y-6">
          ...
          <div className="bg-white shadow-xl shadow-violet-200/50 border border-violet-100 rounded-[2rem] p-8 text-violet-950">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-violet-50/50 shadow-sm border border-violet-100 rounded-2xl p-5 flex flex-col items-center gap-3 text-center">
                <h4 className="font-black text-lg mb-1">🏪 Dueños / Comercios</h4>
                <div className="w-36 h-36 bg-white rounded-xl border border-violet-200 p-1.5 shadow-sm">
                ...
                <p className="text-xs text-slate-500 leading-tight">Ganas <strong className="text-violet-700">50% de la cuota</strong> + 20% regalías de por vida.</p>
                <button onClick={() => { navigator.clipboard.writeText(hostUrl + '?role=dueño&ref=' + currentUser.id); showToast('Enlace copiado', 'success'); }} className="text-[10px] font-black text-violet-700 bg-violet-100 hover:bg-violet-200 px-3 py-1.5 rounded-lg cursor-pointer w-full transition-colors border border-violet-200">📋 Copiar Enlace Comercios</button>
              </div>
              <div className="bg-violet-50/50 shadow-sm border border-violet-100 rounded-2xl p-5 flex flex-col items-center gap-3 text-center">
                ...
                <button onClick={() => { navigator.clipboard.writeText(hostUrl + '?role=vendedor&ref=' + currentUser.id); showToast('Enlace copiado', 'success'); }} className="text-[10px] font-black text-violet-700 bg-violet-100 hover:bg-violet-200 px-3 py-1.5 rounded-lg cursor-pointer w-full transition-colors border border-violet-200">📋 Copiar Enlace Vendedores</button>
              </div>
              <div className="bg-violet-50/50 shadow-sm border border-violet-100 rounded-2xl p-5 flex flex-col items-center gap-3 text-center">
            ...
            <div className="mt-6 pt-6 border-t border-violet-100 flex flex-col sm:flex-row items-center justify-between gap-4">
              <p className="text-xs text-slate-500">Tu ID de Promotora: <span className="font-mono font-black text-violet-700 bg-violet-50 px-2 py-0.5 rounded border border-violet-200">{currentUser.id}</span></p>
              <button onClick={() => setShowCustomerRegister(true)} className="bg-violet-600 text-white px-6 py-3 rounded-xl font-black shadow-md shadow-violet-600/30 hover:bg-violet-700 transition-colors flex items-center gap-2 cursor-pointer text-sm"><UserPlus size={18} /> Registrar Cliente en Vivo</button>
            </div>
          </div>

          <div className="bg-white shadow-xl shadow-violet-200/50 border border-violet-100 rounded-[2rem] p-8">
            <h3 className="text-lg font-black mb-4 text-violet-950">Mis Clientes Afiliados ({myCustomers.length})</h3>
            {myCustomers.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {myCustomers.map((c: any) => (
                  <div key={c.id} className="p-4 border border-violet-100 rounded-xl flex justify-between items-center bg-violet-50/50 hover:bg-violet-50 transition-colors">
                    <div>
                      <p className="font-bold text-sm text-violet-950">{c.name}</p>
          ...
          {!showRegisterVendedor ? (
            <div className="bg-white shadow-xl shadow-violet-200/50 border border-violet-100 rounded-[2rem] p-8 relative overflow-hidden">
              <div className="flex justify-between items-center mb-8 border-b border-violet-100 pb-4">
                <h3 className="text-xl font-black text-violet-950">Vendedores Activados</h3>
                <button onClick={() => setShowRegisterVendedor(true)} className="bg-violet-600 text-white px-6 py-2 rounded-xl font-bold text-sm hover:bg-violet-700 transition-colors shadow-md shadow-violet-600/30 cursor-pointer">+ Nuevo Vendedor</button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 {(db.vendedores || []).filter((v: any) => v.promotoraId === currentUser.id).map((v: any) => (
                   <div key={v.id} className="p-4 border border-violet-100 rounded-xl bg-violet-50/50 hover:bg-violet-50 transition-colors flex justify-between items-center">
                      <div>
                        <p className="font-bold text-violet-950">{v.name}</p>
                        <p className="text-xs text-slate-500">{v.email}</p>
                        {v.clientId && <p className="text-[10px] bg-violet-100 text-violet-700 border border-violet-200 font-bold px-2 py-0.5 rounded-md inline-block mt-1">Ref: {myClients.find((c: any) => c.id === v.clientId)?.company || v.clientId}</p>}
                      </div>
                      <span className="text-[10px] text-violet-600 bg-violet-100 border border-violet-200 px-2 py-1 rounded-full uppercase font-black">Activo</span>
                   </div>
                 ))}
            ...
            <div className="bg-white shadow-xl shadow-violet-200/50 border border-violet-100 rounded-[2rem] p-8 max-w-2xl mx-auto">
              <div className="flex justify-between mb-4 items-center">
                <h3 className="text-xl font-black text-violet-950">Activar Nuevo Vendedor</h3>
                <button onClick={() => setShowRegisterVendedor(false)} className="text-slate-400 hover:text-violet-950 cursor-pointer transition-colors"><X size={20}/></button>
              </div>
              <form onSubmit={(e) => {
                e.preventDefault();
                ...
                <div>
                  ...
                  <input name="vName" required className="w-full bg-white border border-violet-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-violet-300 focus:border-violet-400 placeholder:text-slate-400 text-violet-950 transition-all" placeholder="Ej: Vendedor Alpha" />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-500 block mb-1">Correo Electrónico (Login)</label>
                  <input type="email" name="vEmail" required className="w-full bg-white border border-violet-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-violet-300 focus:border-violet-400 placeholder:text-slate-400 text-violet-950 transition-all" placeholder="vendedor@kfs.com" />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-500 block mb-1">Contraseña</label>
                  <input type="password" name="vPassword" required className="w-full bg-white border border-violet-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-violet-300 focus:border-violet-400 placeholder:text-slate-400 text-violet-950 transition-all" placeholder="*****" />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-500 block mb-1">Asignar a Comercio (Opcional)</label>
                  <select name="vClient" className="w-full bg-white border border-violet-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-violet-300 focus:border-violet-400 text-violet-950 placeholder:text-slate-400 transition-all">
                    <option value="">Independiente (Sin Comercio Fijo)</option>
                    {myClients.map((c: any) => <option key={c.id} value={c.id}>{c.company}</option>)}
                  </select>
                </div>
                <button type="submit" className="w-full bg-violet-600 hover:bg-violet-700 transition-colors text-white py-4 rounded-xl font-black mt-6 shadow-md shadow-violet-600/30 cursor-pointer">Activar y Registrar Vendedor</button>
              </form>
            </div>
          )}
        </div>
      ...
      {showCustomerRegister && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-[2rem] w-full max-w-2xl max-h-[90vh] overflow-y-auto relative p-2 shadow-2xl border border-violet-100">
            <button onClick={() => setShowCustomerRegister(false)} className="absolute top-6 right-6 z-50 text-slate-400 hover:text-violet-950 transition-colors cursor-pointer"><X size={24} /></button>
            <div className="p-4 border-b border-violet-100 mb-4">
              <h3 className="text-xl font-black text-center text-violet-950">Registro Rápido de Cliente</h3>
              <p className="text-xs text-center text-slate-500 mt-1">Este cliente quedará atado a tu ID: <span className="font-mono font-bold text-violet-600 bg-violet-50 px-2 py-0.5 rounded">{currentUser.id}</span></p>
            </div>
