...
export const ClientDashboard = ({ db, setDb, currentUser, addProduct, addExpense, showToast, formatUSD, formatEUR, logout, approveOrder, rejectOrder, dispatchOrder, paySubscription, requestPayout, requestTopUp }: any) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 font-sans text-violet-950 relative">
      {currentUser?.isImpersonated && (
        <div className="bg-amber-500 text-violet-950 px-4 py-3 font-bold text-center flex items-center justify-center gap-4 text-sm shadow-md animate-pulse sticky top-0 z-50 border-b border-amber-600">
          <span>⚠️ MODO IMPERSONACIÓN ACTIVO: Estás controlando el panel de {currentUser.company}</span>
          <button onClick={stopImpersonating} className="bg-violet-950 text-white px-4 py-1.5 rounded-xl text-xs font-black hover:bg-slate-800 transition-colors shadow cursor-pointer">
      ...
      <div className="bg-gradient-to-br from-violet-900 to-violet-950 rounded-b-[3rem] shadow-xl shadow-violet-900/20 pt-6 pb-12 px-6 text-white relative z-10 border-b border-violet-800">
        ...
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 bg-violet-600 rounded-full flex items-center justify-center text-white font-black text-2xl flex-shrink-0 shadow-lg border-4 border-violet-800 relative z-20">
          ...
          <div>
            ...
            <p className="text-violet-300 font-mono text-xs mt-1 truncate max-w-[200px] bg-violet-950/50 inline-block px-2 py-0.5 rounded-md border border-violet-800">{currentUser.email}</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 -mt-6 relative z-20 space-y-6 animate-fade-in">
          ...
          <div className="space-y-6">
            <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-xl shadow-violet-200/50 border border-violet-100 relative overflow-hidden text-violet-950">
              <div className="relative z-10 w-full flex flex-col md:flex-row md:justify-between md:items-end gap-6">
                <div>
                  <p className="text-violet-600 text-xs font-black uppercase tracking-widest mb-4">Ganancia Neta (USD)</p>
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-2 bg-violet-50 px-4 py-2 rounded-xl border border-violet-100 placeholder:text-slate-400">
                      ...
                      >
                        <option value="velocity" className="text-violet-950 bg-white">Flow Velocity (3%)</option>
                        <option value="matrix" className="text-violet-950 bg-white">Flow Matrix (5%)</option>
                        <option value="monopoly" className="text-violet-950 bg-white">Flow Monopoly (10%)</option>
                      </select>
                    </div>
                    <div className="flex items-center gap-2 bg-violet-50 px-4 py-2 rounded-xl border border-violet-100 placeholder:text-slate-400">
                  ...
                  <button onClick={() => setShowExpenseModal(true)} className="bg-violet-600 text-white font-black px-6 py-3 rounded-xl shadow-lg shadow-violet-600/30 hover:scale-105 transition-transform border-none cursor-pointer">
                    Registrar Gasto
                  </button>
                </div>
              </div>
            ...
            <div className="bg-white rounded-[2rem] shadow-xl shadow-violet-200/50 p-8 flex flex-col md:flex-row items-center gap-8 border border-violet-100">
              <div className="flex-1 w-full">
                ...
                <div className="w-full bg-violet-100 h-4 rounded-full overflow-hidden placeholder:text-slate-400">
                  <div className="bg-violet-600 h-full transition-all duration-1000" style={{ width: `${Math.min(((clientInfo?.onboardedUsers || 0) / 50) * 100, 100)}%` }}></div>
                </div>
              ...
              <div className="w-32 h-32 bg-white border border-violet-200 rounded-xl flex items-center justify-center flex-shrink-0 text-center relative p-3 shadow-sm">
        ...
        {activeTab === 'config' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Open / Close */}
            <div className="bg-white rounded-[2rem] shadow-xl shadow-violet-200/50 border border-violet-100 p-6 flex flex-col gap-4">
              <h4 className="font-black text-violet-950 flex items-center gap-2"><Store size={20} className="text-violet-500" /> Estado del Negocio</h4>
              <div className="flex items-center justify-between bg-violet-50 border border-violet-100 rounded-2xl p-4">
                <div>
                  <p className="font-black text-sm text-violet-950">{clientInfo.isOpen !== false ? '🟢 Abierto' : '🔴 Cerrado'}</p>
              ...
              <div className="space-y-2">
                ...
                {['Lun-Vie', 'Sáb', 'Dom'].map(day => (
                  <div key={day} className="flex items-center gap-2">
                    ...
                    <input type="time" defaultValue="08:00" className="bg-white border border-violet-200 shadow-sm rounded-lg px-2 py-1 text-xs flex-1 text-violet-950" />
                    <span className="text-xs text-slate-400">–</span>
                    <input type="time" defaultValue="18:00" className="bg-white border border-violet-200 shadow-sm rounded-lg px-2 py-1 text-xs flex-1 text-violet-950" />
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Delivery zone */}
            <div className="bg-white rounded-[2rem] shadow-xl shadow-violet-200/50 border border-violet-100 p-6 flex flex-col gap-4">
              <h4 className="font-black text-violet-950 flex items-center gap-2"><Truck size={20} className="text-emerald-500" /> Zona de Delivery</h4>
              <div className="flex flex-col gap-3">
                <div>
        ...
        {activeTab === 'resumen' && (
          <div className="bg-gradient-to-r from-violet-950 to-violet-900 text-white p-6 md:p-8 rounded-[2rem] shadow-xl shadow-violet-900/20 relative overflow-hidden border border-violet-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
          {activeTab === 'inventario' && (
            <>
              <button onClick={() => setShowAddModal(true)} className="bg-white border border-violet-100 p-8 rounded-[2rem] shadow-xl shadow-violet-200/50 hover:shadow-2xl hover:shadow-violet-300/50 flex flex-col items-center justify-center gap-5 transition-all cursor-pointer">
                <div className="w-16 h-16 bg-violet-50 rounded-full flex items-center justify-center border border-violet-200">
                  <Package size={32} className="text-violet-600" />
                </div>
              ...
              <button onClick={() => setShowTicketModal(true)} className="bg-gradient-to-br from-violet-600 to-violet-700 text-white p-8 rounded-[2rem] shadow-xl shadow-violet-600/30 border-none hover:shadow-2xl hover:shadow-violet-600/40 flex flex-col items-center justify-center gap-5 transition-all relative overflow-hidden group cursor-pointer">
        ...
        {activeTab === 'resumen' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
            <div className="bg-white p-6 md:p-8 rounded-[2rem] shadow-xl shadow-violet-200/50 border border-violet-100 h-full">
              <h3 className="font-black text-xl text-violet-950 mb-6 flex items-center gap-2"><TrendingUp className="text-violet-500" /> Rendimiento de Ventas</h3>
            ...
            <div className="bg-white p-6 md:p-8 rounded-[2rem] shadow-xl shadow-violet-200/50 border border-violet-100 h-full flex flex-col">
              <h3 className="text-xl font-black text-violet-950 mb-6 flex items-center gap-2"><Star className="text-amber-400" /> Productos Estrella</h3>
              <div className="space-y-4">
                {myProducts.slice(0, 4).map((p: any, i: number) => (
                  <div key={i} className="flex justify-between items-center bg-violet-50 border border-violet-100 p-4 rounded-xl">
                    <div className="flex items-center gap-3">
                      <div className="bg-violet-200 text-violet-700 font-black h-8 w-8 rounded-full flex items-center justify-center">#{i + 1}</div>
                      <span className="font-bold text-violet-950">{p.name}</span>
                    </div>
        ...
        {activeTab === 'resumen' && (
            <div className="bg-gradient-to-r from-violet-600 to-violet-700 text-white p-6 md:p-8 rounded-[2rem] shadow-xl shadow-violet-600/30 relative overflow-hidden border border-violet-500 h-full">
        ...
        {activeTab === 'personal' && (
          <div className="bg-white p-6 md:p-8 rounded-[2rem] shadow-xl shadow-violet-200/50 border border-violet-100 w-full">
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-6 border-b border-violet-100 pb-4">
              <h3 className="font-black text-xl text-violet-950 flex items-center gap-2"><Users className="text-violet-500" /> Control de Empleados</h3>
              <div className="flex gap-4">
                <div className="relative w-48">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                  <input type="text" placeholder="Buscar vendedor..." className="w-full bg-violet-50 border border-violet-100 rounded-xl pl-10 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-600 placeholder:text-slate-400" value={searchVendedor} onChange={e => setSearchVendedor(e.target.value)} />
                </div>
                <button onClick={() => setShowAddVendedor(true)} className="text-sm font-bold text-white bg-violet-900 px-4 py-2 rounded-lg hover:bg-violet-800 transition-colors cursor-pointer">+ Añadir Vendedor</button>
              </div>
            </div>
            <div className="space-y-3">
              {myVendedores.filter((v: any) => v.name?.toLowerCase().includes(searchVendedor.toLowerCase()) || v.email?.toLowerCase().includes(searchVendedor.toLowerCase())).map((v: any) => (
                <div key={v.id} className="flex justify-between items-center p-4 bg-violet-50 rounded-2xl border border-violet-100">
                  <div className="flex items-center gap-3">
                    {v.avatar ? (
                      <img src={v.avatar} className="w-10 h-10 rounded-full object-cover border border-violet-200" alt="Avatar" />
                    ) : (
                      <div className="w-10 h-10 rounded-full bg-violet-100 text-violet-700 font-black text-xs flex items-center justify-center border border-violet-200">
                        {v.name.slice(0, 2).toUpperCase()}
                      </div>
                    )}
                    <div className="flex flex-col">
                      <span className="font-bold text-violet-950">{v.name}</span>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => setShowPayrollModal(v)} className="bg-violet-100 text-violet-700 border border-violet-200 text-[10px] font-black px-3 py-1.5 rounded-lg hover:bg-violet-600 hover:text-white transition-colors cursor-pointer uppercase tracking-wider">
                      Liquidar Nómina
                    </button>
                  </div>
                </div>
        ...
        {activeTab === 'config' && (
          <div className="bg-white p-6 md:p-8 rounded-[2rem] shadow-xl shadow-violet-200/50 border border-violet-100 relative overflow-hidden w-full">
            <div className="absolute top-0 right-0 w-40 h-40 bg-violet-50 rounded-bl-[100px] -z-10"></div>
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-6 border-b border-violet-100 pb-4">
              <h3 className="font-black text-xl text-violet-950 flex items-center gap-2"><DollarSign className="text-violet-600" /> Datos Oficiales de Liquidación {KFS_BRAND.productAcronym}</h3>
              <span className="bg-violet-50 text-violet-600 text-xs font-black px-3 py-1 rounded-full uppercase tracking-wider border border-violet-100">Transferencia Directa</span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="flex flex-col bg-slate-50 p-4 rounded-xl border border-violet-100">
                <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1">Entidad Bancaria</span>
                <span className="font-black text-sm text-violet-950">Banco Nacional de Crédito (BNC)</span>
              </div>
              <div className="flex flex-col bg-slate-50 p-4 rounded-xl border border-violet-100">
                <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1">Teléfono (Pago Móvil)</span>
                <span className="font-mono font-bold text-sm text-violet-950">0412-7740041</span>
              </div>
              <div className="flex flex-col bg-slate-50 p-4 rounded-xl border border-violet-100">
                <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1">Cédula de Identidad</span>
                <span className="font-mono font-bold text-sm text-violet-950">V-25.218.648</span>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'resumen' && businessPreset !== "AXIS-ONLY" && (
          <div className="bg-gradient-to-br from-violet-900 to-slate-900 text-white p-6 md:p-8 rounded-[2rem] shadow-2xl shadow-violet-900/40 relative overflow-hidden border border-violet-800 animate-fade-in w-full">
            ...
            <div className="flex flex-col md:flex-row justify-between items-stretch gap-6 relative z-10">
              ...
              <div className="flex-1 flex flex-col justify-between p-6 bg-white/5 rounded-2xl border border-white/10 relative">
                <div className="space-y-2">
                  <span className="text-violet-300 text-[10px] font-black uppercase tracking-widest block font-mono">Cierre de Caja & Liquidación</span>
                ...
                <div className="mt-6 pt-4 border-t border-white/10 flex justify-between items-baseline">
                  <div>
                    <span className="text-[10px] text-violet-300 block font-bold">TOTAL A REPORTAR:</span>
                  ...
                  <button
                    ...
                    className="bg-violet-600 text-white font-black text-xs px-5 py-3 rounded-xl shadow-lg shadow-violet-600/30 hover:scale-105 transition-transform flex items-center gap-1.5 cursor-pointer border-none"
                  >
        ...
        {activeTab === 'personal' && (
          <div className="bg-white p-8 rounded-[2rem] shadow-xl shadow-violet-200/50 border border-violet-100 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {/* Emitir Vale Form */}
              <div className="bg-slate-50 p-6 rounded-2xl border border-violet-100 space-y-4">
                ...
                <form onSubmit={e => {
                  ...
                  <input required name="recipient" placeholder="Telf. Cliente CRM o Nombre Vendedor" className="w-full bg-white border border-violet-200 rounded-xl px-4 py-2.5 text-xs text-violet-950 focus:outline-none" />
                  <select name="type" className="w-full bg-white border border-violet-200 rounded-xl px-4 py-2.5 text-xs text-violet-950 focus:outline-none font-bold">
                    <option value="credito_cliente">Crédito a Cliente (CRM)</option>
                    <option value="adelanto_nomina">Adelanto de Nómina</option>
                  </select>
                  <div className="grid grid-cols-2 gap-2">
                    <input required type="number" step="0.01" name="amount" placeholder="Monto ($)" className="w-full bg-white border border-violet-200 rounded-xl px-3 py-2.5 text-xs text-violet-950 focus:outline-none" />
                    <select name="surcharge" className="w-full bg-white border border-violet-200 rounded-xl px-3 py-2.5 text-xs text-violet-950 focus:outline-none font-bold">
                  ...
                  <input required type="date" name="dueDate" className="w-full bg-white border border-violet-200 rounded-xl px-4 py-2.5 text-xs text-violet-950 focus:outline-none font-mono" />
                  <button type="submit" className="w-full py-3 bg-violet-900 hover:bg-slate-800 text-white font-black rounded-xl text-xs shadow-md transition-all cursor-pointer">
              ...
              <div className="md:col-span-2 space-y-4">
                ...
                <div className="overflow-x-auto max-h-60 border border-violet-100 rounded-xl bg-slate-50/50 p-2">
                  <table className="w-full text-xs text-left">
                    <thead className="bg-violet-50 text-slate-500 uppercase text-[9px] font-black sticky top-0">
                      <tr>
                        <th className="py-2.5 px-3 rounded-l-lg border-b border-violet-100">Vale / Beneficiario</th>
                        <th className="py-2.5 px-2 border-b border-violet-100">Tipo</th>
                        <th className="py-2.5 px-2 border-b border-violet-100">Vencimiento</th>
                        <th className="py-2.5 px-2 border-b border-violet-100">Total Adeudado</th>
                        <th className="py-2.5 px-3 text-right rounded-r-lg border-b border-violet-100">Acciones</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(db.vales || []).map((v: any) => (
                        <tr key={v.id} className="border-b border-violet-100/50 hover:bg-white transition-colors">
                          <td className="py-3 px-3">
                            <span className="font-mono font-black text-violet-950 block">{v.id}</span>
        ...
        {activeTab === 'resumen' && (
          <div className="bg-white p-8 rounded-[2rem] shadow-xl shadow-violet-200/50 border border-violet-100 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-violet-900 text-white p-6 rounded-2xl border border-violet-800 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-violet-500/10 rounded-full blur-3xl" />
                <h4 className="text-[10px] font-black uppercase text-violet-300 mb-2 font-mono">Caja & Ganancias Disponibles</h4>
                <p className="text-3xl font-black mb-1">${formatUSD(currentUser.salesUSD || 0)} <span className="text-sm font-light text-violet-200/70">/ $1,000</span></p>
                <div className="w-full bg-white/10 h-2 rounded-full overflow-hidden mt-4 mb-4">
                  <div className="bg-violet-400 h-full" style={{ width: `${Math.min(100, ((currentUser.salesUSD || 0) / 1000) * 100)}%` }} />
                </div>
                <button
              ...
              <div className="bg-slate-50 p-6 rounded-2xl border border-violet-100">
                <h4 className="text-[10px] font-black uppercase text-slate-500 mb-2 font-mono">Categorías Fuertes</h4>
                <ul className="space-y-2 mt-4 text-sm font-bold text-violet-950">
          ...
          <div className="space-y-6">
            ...
            <form onSubmit={e => {
              ...
              <div className="space-y-2">
                ...
                <input name="zinli" defaultValue={currentUser.paymentMethods?.zinli || ""} placeholder="correo@zinli.com" className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 text-sm text-violet-950 focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 placeholder:text-slate-400" />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest font-mono block">Wally Tech</label>
                <input name="wallyTech" defaultValue={currentUser.paymentMethods?.wallyTech || ""} placeholder="Usuario o Teléfono" className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 text-sm text-violet-950 focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 placeholder:text-slate-400" />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest font-mono block">AirTM</label>
                <input name="airtm" defaultValue={currentUser.paymentMethods?.airtm || ""} placeholder="correo@airtm.com" className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 text-sm text-violet-950 focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 placeholder:text-slate-400" />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest font-mono block">Ubbi App</label>
                <input name="ubbiApp" defaultValue={currentUser.paymentMethods?.ubbiApp || ""} placeholder="Usuario Ubbi" className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 text-sm text-violet-950 focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 placeholder:text-slate-400" />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest font-mono block">Binance Pay (Pay ID)</label>
                <input name="binance" defaultValue={currentUser.paymentMethods?.binance || ""} placeholder="ID de Binance" className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 text-sm text-violet-950 focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 placeholder:text-slate-400" />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-slate-50 p-6 rounded-2xl border border-violet-100 space-y-4">
                ...
                <form onSubmit={e => {
                  ...
                  <input required name="posName" placeholder="Nombre POS (Ej: Pax A920 - Caja 1)" className="w-full bg-white border border-violet-200 rounded-xl px-4 py-2.5 text-xs text-violet-950 focus:outline-none" />

                  <div className="grid grid-cols-2 gap-2">
                    <select name="connectionType" className="w-full bg-white border border-violet-200 rounded-xl px-2 py-2.5 text-[10px] text-violet-950 focus:outline-none font-bold">
                      <option value="TCP_IP">Red Local (IP)</option>
                      <option value="SERIAL">Puerto COM (Serial)</option>
                    </select>
                    <input required name="connectionInfo" placeholder="IP (192...) o COM3" className="w-full bg-white border border-violet-200 rounded-xl px-2 py-2.5 text-[10px] text-violet-950 focus:outline-none font-mono" />
                  </div>

                  <div>
                    <label className="text-[9px] font-bold text-slate-400 block mb-1 uppercase tracking-widest">Cajero Asignado</label>
                    <select name="assignedVendedorId" className="w-full bg-white border border-violet-200 rounded-xl px-4 py-2.5 text-xs text-violet-950 focus:outline-none font-bold">
              ...
              <div className="md:col-span-2 space-y-4">
                ...
                <div className="overflow-x-auto max-h-60 border border-violet-100 rounded-xl bg-slate-50/50 p-2">
                  <table className="w-full text-xs text-left">
                    <thead className="bg-violet-50 text-slate-500 uppercase text-[9px] font-black sticky top-0">
                      <tr>
                        <th className="py-2.5 px-3 rounded-l-lg border-b border-violet-100">POS Físico / Canal</th>
                        <th className="py-2.5 px-2 border-b border-violet-100">Cajero</th>
                        <th className="py-2.5 px-2 border-b border-violet-100">Telemetría</th>
                        <th className="py-2.5 px-2 border-b border-violet-100">Facturación Real</th>
                        <th className="py-2.5 px-3 text-right rounded-r-lg border-b border-violet-100">Acciones</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(db.posTerminals || []).filter((p: any) => p.clientId === currentUser.id).map((p: any) => {
                        ...
                        return (
                          <tr key={p.id} className="border-b border-violet-100/50 hover:bg-white transition-colors">
                            <td className="py-3 px-3">
                              <span className="font-bold text-violet-950 block">{p.name}</span>
        ...
        {activeTab === 'inventario' && (
          <div className="space-y-6">
            {(myOrders.length > 0 || myPendingDispatch.length > 0) && (
              <div className="bg-white p-8 rounded-[2rem] shadow-xl shadow-violet-200/50 border border-amber-200 bg-amber-50/30">
                <h3 className="font-black text-xl text-violet-950 mb-6 flex items-center gap-2 text-amber-600">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      ...
                      return (
                        <div key={order.id} className="flex flex-col md:flex-row justify-between items-start md:items-center p-5 bg-white rounded-2xl border border-amber-100 shadow-sm gap-4 animate-fade-in">
                          <div>
                            ...
                            <h4 className="font-bold text-violet-950">{product?.name || "Producto Desconocido"}</h4>
                  ...
                  {myPendingDispatch.length > 0 && (
                    <div className="lg:col-span-2 space-y-4 mt-8 lg:mt-0 lg:border-t-0 border-t border-amber-100 pt-8 lg:pt-0">
                      <h4 className="font-bold text-violet-950 mb-4 flex items-center gap-2">
                        <Package className="text-violet-500" /> Listo para Empacar / Despachar ({myPendingDispatch.length})
                      </h4>
                      {myPendingDispatch.map((tx: any) => {
                        ...
                        return (
                          <div key={tx.id} className="flex flex-col p-5 bg-violet-50 rounded-2xl border border-violet-100 shadow-sm gap-4 animate-fade-in">
                            <div className="flex justify-between items-start flex-wrap gap-2">
                              <div>
                                <span className="bg-violet-200 text-violet-800 text-[10px] font-black px-2 py-1 rounded-full uppercase tracking-wider mb-2 inline-block">Pago Aprobado</span>
                                <h4 className="font-bold text-violet-950">{product?.name || "Producto Desconocido"}</h4>
                                <p className="text-sm text-slate-600 font-mono mt-1">Teléfono: <span className="font-bold">{tx.customerPhone}</span></p>
                              </div>
                              <div className="flex gap-2">
                                <button onClick={() => { dispatchOrder(tx.id); assignDeliveryToOrder(tx.id, currentUser.id); }} className="px-4 py-2 bg-violet-600 text-white rounded-xl hover:bg-violet-700 transition-colors cursor-pointer flex items-center justify-center gap-2 font-bold shadow-md text-sm">
                            ...
                            {assignedRider && (
                              <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3">
                                ...
                                <p className="font-bold text-sm text-violet-950">{assignedRider.name}</p>
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="bg-white border border-violet-100 rounded-2xl p-5 mt-4">
                            <h4 className="font-black text-violet-950 text-sm mb-3 flex items-center gap-2">
                              <Truck size={16} className="text-violet-500" /> Mis Riders ({myRiders.length}/2)
                            </h4>
                            {myRiders.length === 0 ? (
                              ...
                              <div className="space-y-2 mb-3">
                                {myRiders.map((r: any) => (
                                  <div key={r.id} className="flex items-center justify-between bg-violet-50 border border-violet-100 rounded-xl p-3">
                                    <div>
                                      <p className="font-black text-sm text-violet-950">{r.name}</p>
                              ...
                              <div>
                                ...
                                <div className="space-y-1">
                                  {availableRiders.slice(0, 5).map((r: any) => (
                                    <button key={r.id} onClick={() => assignRiderToBusiness(r.id, currentUser.id)} className="w-full text-left px-3 py-2 bg-violet-50 border border-violet-100 rounded-xl hover:bg-violet-100 hover:border-violet-200 transition-colors cursor-pointer placeholder:text-slate-400">
                                      <span className="font-bold text-sm text-violet-950">{r.name}</span>
                                      <span className="text-[10px] text-slate-500 ml-2">{r.email}</span>
                                      <span className="float-right text-[10px] text-violet-600 font-black">+ Añadir</span>
                                    </button>
                                  ))}
                                </div>
                              </div>
                  ...
                  <div className="bg-gradient-to-br from-violet-900 to-slate-900 border border-violet-800 rounded-[2rem] p-6 text-white relative overflow-hidden shadow-2xl shadow-violet-900/40 flex flex-col justify-between text-left">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-violet-500/10 rounded-full blur-2xl pointer-events-none"></div>
                    <div>
                      <div className="flex items-center gap-3 mb-4">
                        <div className="p-2.5 rounded-xl bg-violet-800 border border-violet-700 text-violet-400">
                          <Bell size={20} className="animate-pulse" />
                        </div>
                        <div>
                          <h4 className="font-black text-sm tracking-wide text-white">CONCILIADOR SMS</h4>
                          <p className="text-[9px] text-violet-400 font-mono uppercase tracking-widest">Tecnología Inteligente {KFS_BRAND.productAcronym}</p>
                        </div>
                      </div>

                      <p className="text-[11px] text-violet-200/70 mb-4 leading-relaxed">
                      ...
                      <textarea
                        ...
                        className="w-full h-24 bg-black/20 border border-violet-800 rounded-xl p-3 text-xs font-mono text-violet-100 focus:outline-none focus:border-violet-500 placeholder:text-violet-700 resize-none leading-relaxed"
                      />
                    </div>
                    ...
                    <button
                      ...
                      className="w-full mt-4 py-3 rounded-xl font-black text-xs text-white bg-violet-600 hover:bg-violet-500 border-none transition-all shadow-lg active:scale-95 flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                      ⚡ Conciliar SMS Real
                    </button>
                  </div>
            ...
            <div className="bg-white p-8 rounded-[2rem] shadow-xl shadow-violet-200/50 border border-violet-100">
              ...
              <div className="space-y-3">
                {myExpenses.map((e: any) => (
                  <div key={e.id} className="flex justify-between items-center p-4 bg-violet-50 rounded-2xl border border-violet-100">
                    <span className="font-bold text-violet-950">{e.description}</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              ...
              {businessPreset !== "AXIS-ONLY" ? (
                <div className="bg-white p-8 rounded-[2rem] shadow-xl shadow-violet-200/50 border border-rose-100 bg-rose-50/20">
                  <h3 className="font-black text-xl text-violet-950 mb-6 flex items-center gap-2 text-rose-500">
                  ...
                  <div className="space-y-3">
                    {lowStockProducts.map((p: any) => (
                      <div key={p.id} className="flex justify-between items-center p-4 bg-white rounded-2xl border border-rose-100 shadow-sm">
                        <div className="flex flex-col">
                          <span className="font-bold text-violet-950">{p.name} <span className="text-[10px] font-black text-white bg-emerald-500 px-2 py-0.5 rounded-full ml-2">Alerta Verde</span></span>
                    ...
                    {stagnantProducts.map((p: any) => (
                      <div key={`stg-${p.id}`} className="flex justify-between items-center p-4 bg-rose-50 rounded-2xl border border-rose-200 shadow-sm">
                        <div className="flex flex-col">
                          <span className="font-bold text-violet-950">{p.name} <span className="text-[10px] font-black text-white bg-rose-500 px-2 py-0.5 rounded-full ml-2">Alerta Roja</span></span>
              ...
              <div className="bg-white p-8 rounded-[2rem] shadow-xl shadow-violet-200/50 border border-violet-100">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                  ...
                  <div className="flex items-center gap-3 bg-violet-50 px-4 py-2 rounded-xl border border-violet-100">
                ...
                <div className="space-y-3">
                  {myCrm.map((c: any) => (
                    <div key={c.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 bg-violet-50 rounded-2xl border border-violet-100 gap-4">
                      <div className="flex flex-col">
                        <span className="font-bold text-violet-950 font-mono">{c.phone} {c.name && <span className="text-slate-500 font-sans font-medium text-xs ml-1">({c.name})</span>}</span>
                      ...
                      <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
                        <div className="text-right">
                          ...
                          {c.kfsPoints > 0 && <span className="text-[10px] font-bold text-violet-600 bg-violet-100 border border-violet-200 px-2 py-0.5 rounded-full">{c.kfsPoints.toFixed(1)} Pts</span>}
                        </div>
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="bg-violet-900 text-white p-2 rounded-xl hover:bg-slate-800 transition-colors shadow-sm flex items-center justify-center"
            ...
            {myZReports.length > 0 && (
              <div className="bg-white p-8 rounded-[2rem] shadow-xl shadow-violet-200/50 border border-violet-100">
                <h3 className="font-black text-xl text-violet-950 mb-6 flex items-center gap-2">
                  <Lock className="text-violet-950" /> Cortes de Caja (Reportes Z)
                </h3>
                <div className="space-y-3">
                  {myZReports.map((z: any) => {
                    const vendedor = myVendedores.find((v: any) => v.id === z.vendedorId);
                    return (
                      <div key={z.id} className="flex justify-between items-center p-5 bg-violet-50 rounded-2xl border border-violet-100">
                        <div className="flex flex-col">
                          <span className="font-black text-violet-950 uppercase tracking-wider text-sm">{vendedor?.name || "Vendedor"} - {new Date(z.timestamp).toLocaleDateString()}</span>
                          <span className="text-xs text-slate-500">{z.txCount} transacciones procesadas</span>
                        </div>
                        <span className="font-black text-xl text-violet-950">{formatUSD(z.totalUSD)}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            <div>
              <h3 className="font-black text-xl text-violet-950 mb-6 pl-2">Inventario en {KFS_BRAND.modules.marketplace}</h3>
              <div className="grid grid-cols-1 min-[400px]:grid-cols-2 md:grid-cols-4 gap-4">
                {myProducts.map((p: any) => {
                  ...
                  return (
                    <div key={p.id} className={`bg-white rounded-2xl shadow-sm overflow-hidden border transition-all ${marginVulnerable ? "border-rose-200 shadow-[0_0_15px_rgba(244,63,94,0.05)]" : "border-violet-100"}`}>
                      ...
                      <div className="p-4 space-y-3">
                        <div>
                          <h4 className="font-bold text-sm truncate text-violet-950 mb-0.5">{p.name}</h4>
                          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{p.category || "General"}</span>
                        </div>

                        <div className="grid grid-cols-3 gap-1 bg-violet-50 p-2.5 rounded-xl border border-violet-100 text-[10px] font-mono leading-tight">
                          ...
                          <div className="flex flex-col text-slate-500 text-center">
                            <span>Costo:</span>
                            <span className="text-violet-950 font-bold">{hasCost ? formatUSD(p.costUSD) : "N/D"}</span>
                          </div>
                        ...
                        <div className="flex justify-between items-center pt-1 border-t border-violet-100 mt-2">
                          <div>
                            <p className="text-violet-950 font-black text-sm">{formatUSD(p.priceUSD)}</p>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[99999] flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white rounded-[2rem] w-full max-w-md p-8 shadow-2xl">
              ...
              <form onSubmit={submitProduct} className="space-y-4">
                <div className="flex gap-2">
                  <input type="text" placeholder="Código de Barras (Opcional)" value={newProd.barcode} onChange={e => setNewProd({ ...newProd, barcode: e.target.value })} className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 focus:outline-none focus:ring-1 focus:ring-violet-500 font-mono text-violet-950 placeholder:text-slate-400" />
                ...
                <input required type="text" placeholder="Nombre del Artículo" value={newProd.name} onChange={e => setNewProd({ ...newProd, name: e.target.value })} className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 focus:outline-none focus:ring-1 focus:ring-violet-500 font-bold text-violet-950 placeholder:text-slate-400" />
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    <label className="text-[9px] font-black uppercase text-slate-400 block mb-1">Costo Insumo</label>
                    <input required type="number" step="0.01" placeholder="Costo ($)" value={newProd.cost} onChange={e => setNewProd({ ...newProd, cost: e.target.value })} className="w-full bg-violet-50 border border-violet-100 rounded-xl px-3 py-3 focus:outline-none focus:ring-1 focus:ring-violet-500 font-bold text-violet-950 placeholder:text-slate-400" />
                  </div>
                  <div>
                    <label className="text-[9px] font-black uppercase text-slate-400 block mb-1">Precio Venta</label>
                    <input required type="number" step="0.01" placeholder="Precio ($)" value={newProd.price} onChange={e => setNewProd({ ...newProd, price: e.target.value })} className="w-full bg-violet-50 border border-violet-100 rounded-xl px-3 py-3 focus:outline-none focus:ring-1 focus:ring-violet-500 font-black text-violet-950 text-center placeholder:text-slate-400" />
                  </div>
                  <div>
                    <label className="text-[9px] font-black uppercase text-slate-400 block mb-1">Stock</label>
                    <input required type="number" placeholder="Cant" value={newProd.stock} onChange={e => setNewProd({ ...newProd, stock: e.target.value })} className="w-full bg-violet-50 border border-violet-100 rounded-xl px-3 py-3 focus:outline-none focus:ring-1 focus:ring-violet-500 font-bold text-violet-950 text-center placeholder:text-slate-400" />
                  </div>
                </div>
                <select value={newProd.category} onChange={e => setNewProd({ ...newProd, category: e.target.value })} className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 focus:outline-none focus:ring-1 focus:ring-violet-500 font-bold text-violet-950 placeholder:text-slate-400">
                ...
                <textarea placeholder="Descripción del producto (Opcional)" value={newProd.description} onChange={e => setNewProd({ ...newProd, description: e.target.value })} className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 focus:outline-none focus:ring-1 focus:ring-violet-500 font-medium text-violet-950 text-sm h-20 resize-none placeholder:text-slate-400" />
                ...
                <div className="flex gap-3 pt-4">
                  <button type="button" onClick={() => setShowAddModal(false)} className="w-1/3 py-3 rounded-xl bg-violet-100 font-bold text-violet-700 cursor-pointer">Cancelar</button>
        ...
        {showAddVendedor && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[99999] flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white rounded-[2rem] w-full max-w-md p-8 shadow-2xl">
              <h3 className="text-2xl font-black mb-6 text-violet-950">Nuevo Empleado</h3>
              <form onSubmit={handleAddVendedor} className="space-y-4">
                <div className="flex flex-col items-center gap-2 mb-4">
                  <label className="relative w-16 h-16 rounded-full border-2 border-dashed border-violet-200 cursor-pointer overflow-hidden flex items-center justify-center bg-slate-50 hover:bg-violet-50 transition-colors group">
                    ...
                    {newVendedor.avatar ? (
                      ...
                      <div className="text-center text-slate-400 group-hover:text-violet-600 transition-colors">
                ...
                <input required type="text" placeholder="Nombre del Vendedor" value={newVendedor.name} onChange={e => setNewVendedor({ ...newVendedor, name: e.target.value })} className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 focus:outline-none focus:ring-1 focus:ring-violet-500 text-violet-950 placeholder:text-slate-400" />
                <input required type="email" placeholder="Correo (Usuario de Acceso)" value={newVendedor.email} onChange={e => setNewVendedor({ ...newVendedor, email: e.target.value })} className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 focus:outline-none focus:ring-1 focus:ring-violet-500 text-violet-950 placeholder:text-slate-400" />
                <input required type="password" placeholder="Clave de Acceso" value={newVendedor.password} onChange={e => setNewVendedor({ ...newVendedor, password: e.target.value })} className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 focus:outline-none focus:ring-1 focus:ring-violet-500 text-violet-950 placeholder:text-slate-400" />
                <div className="flex gap-3 pt-4">
                  <button type="button" onClick={() => setShowAddVendedor(false)} className="w-1/3 py-3 rounded-xl bg-violet-100 font-bold text-violet-700 cursor-pointer">Cancelar</button>
        ...
        {showExpenseModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[99999] flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white rounded-[2rem] w-full max-w-md p-8 shadow-2xl">
              <h3 className="text-2xl font-black mb-6 text-violet-950">Registrar Gasto (Egreso)</h3>
              <form onSubmit={(e) => {
                e.preventDefault();
                addExpense({
                ...
                <input required type="text" placeholder="Concepto (Ej. Alquiler, Proveedor)" value={newExpense.description} onChange={e => setNewExpense({ ...newExpense, description: e.target.value })} className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 focus:outline-none focus:ring-1 focus:ring-violet-500 text-violet-950 placeholder:text-slate-400" />
                <input required type="number" step="0.01" placeholder="Monto Total (USD)" value={newExpense.amountUSD} onChange={e => setNewExpense({ ...newExpense, amountUSD: e.target.value })} className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 focus:outline-none focus:ring-1 focus:ring-violet-500 text-violet-950 placeholder:text-slate-400" />
                <div className="flex gap-3 pt-4">
                  <button type="button" onClick={() => setShowExpenseModal(false)} className="w-1/3 py-3 rounded-xl bg-violet-100 font-bold text-violet-700 cursor-pointer">Cancelar</button>
        ...
        {activeManual && (
          <div className="fixed inset-0 bg-black/80 z-[99999] flex items-center justify-center p-4 backdrop-blur-md animate-fade-in">
            <div className="bg-white text-violet-950 rounded-[2rem] w-full max-w-2xl max-h-[85vh] overflow-y-auto relative p-8 shadow-2xl border-4 border-violet-900">
              <button onClick={() => setActiveManual(null)} className="absolute top-6 right-6 text-slate-500 hover:text-violet-950 transition-colors cursor-pointer bg-slate-100 p-2 rounded-full hover:bg-slate-200">
                <X size={20} />
              </button>
                ...
                <div>
                  ...
                  <div className="space-y-5 text-slate-700 text-sm leading-relaxed">
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                      <p className="font-black text-violet-950 mb-1">1. ¿Qué es {KFS_BRAND.productAcronym} OS?</p>
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                      <p className="font-black text-violet-950 mb-1">2. Control de Inventario:</p>
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                      <p className="font-black text-violet-950 mb-1">3. Control de Empleados (Vendedores):</p>
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                      <p className="font-black text-violet-950 mb-1">4. Liquidación y Tarifas:</p>
              ...
              {activeManual === 'benefits' && (
                <div>
                  <h2 className="text-2xl font-black mb-6 flex items-center gap-2"><Star className="text-violet-600" size={28} /> Whitepaper de Beneficios {KFS_BRAND.productAcronym}</h2>
                  <div className="space-y-5 text-slate-700 text-sm leading-relaxed">
                    <div className="bg-violet-50 border border-violet-200 p-4 rounded-xl mb-4">
                      <p className="font-black text-violet-800 text-xs uppercase tracking-widest mb-1">El Ecosistema Financiero</p>
                      <p className="text-violet-700 text-xs">Ahorros masivos al eliminar software de terceros obsoleto.</p>
                    </div>
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                      <p className="font-black text-violet-950 mb-1">1. E-Commerce {KFS_BRAND.modules.marketplace} Gratuito:</p>
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                      <p className="font-black text-violet-950 mb-1">2. Sincro-Shield Fiscal Gratuito:</p>
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                      <p className="font-black text-violet-950 mb-1">3. Conciliación Automática SMS:</p>
        ...
        {showTicketModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[99999] flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white rounded-[2rem] max-w-md w-full p-8 space-y-4 shadow-2xl border border-violet-100">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-black text-violet-950">Crear Ticket SOS</h3>
              ...
              <input type="text" placeholder="Asunto (Ej: Lector no lee)" value={ticketSubject} onChange={e => setTicketSubject(e.target.value)} className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 font-bold focus:outline-none focus:border-violet-500 placeholder:text-slate-400" />
              <textarea placeholder="Describe el problema..." value={ticketMsg} onChange={e => setTicketMsg(e.target.value)} className="w-full bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 h-32 resize-none focus:outline-none focus:border-violet-500 placeholder:text-slate-400" />
        ...
        {activeScreenshot && (
          <div className="fixed inset-0 bg-black/80 z-[99999] flex items-center justify-center p-4 backdrop-blur-md animate-fade-in">
            <div className="bg-violet-900 border border-violet-600/30 rounded-[2.5rem] w-full max-w-lg p-6 shadow-2xl relative flex flex-col items-center">
              <button onClick={() => setActiveScreenshot(null)} className="absolute top-6 right-6 text-violet-200 hover:text-white transition-colors cursor-pointer bg-white/10 p-2 rounded-full">
                <X size={20} />
              </button>
      ...
      {showPayrollModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-[99999] p-4">
          <div className="bg-white p-6 md:p-8 rounded-[2rem] shadow-2xl max-w-md w-full animate-scale-in border border-violet-100">
            <h3 className="text-xl font-black mb-1 flex items-center gap-2 text-violet-950"><DollarSign className="text-violet-500" /> Liquidación de Nómina</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" step="0.01" className="w-full bg-violet-50 border border-violet-100 rounded-xl pl-8 pr-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-violet-500 placeholder:text-slate-400" placeholder="0.00" value={payrollBaseSalary} onChange={(e) => setPayrollBaseSalary(e.target.value)} />
                </div>
              </div>
              ...
              <div className="flex justify-between items-center bg-violet-900 p-4 rounded-xl text-white">
                <span className="text-xs font-bold uppercase tracking-wider text-violet-200">Total Neto a Pagar</span>
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => { setShowPayrollModal(null); setPayrollBaseSalary(""); }} className="flex-1 px-4 py-3 border border-violet-100 text-violet-700 text-sm font-bold rounded-xl hover:bg-violet-50 transition-colors cursor-pointer">
                Cancelar
              </button>
              <button
                onClick={() => {
                ...
                className="flex-1 px-4 py-3 bg-violet-900 text-white text-sm font-black uppercase tracking-wider rounded-xl hover:bg-slate-800 transition-colors cursor-pointer shadow-lg"
              >
                Aprobar Pago
              </button>
            </div>
