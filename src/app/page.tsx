...
export default function Home() {
  ...
  // Rendering loading screen while booting
  ...
  const Toast = () => {
    ...
    return (
      <div className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-full shadow-2xl backdrop-blur-md font-bold text-sm transition-all duration-300 flex items-center gap-2 ${toast.type === "error" ? "bg-red-500/90 text-white border border-red-400" : "bg-[#C5A184]/90 text-[#0A1128] border border-white/20"}`}>
    ...
    return (
      <div className="min-h-screen flex flex-col bg-[#0A1128] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#141E3A] to-[#0A1128] font-sans">
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-8 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
  ...
  // 4. CLIENT DASHBOARD (COMPANY OWNER)
  ...
  // 5. VENDEDOR DASHBOARD (OPERATIVE USER)
  const VendedorDashboard = () => {
    ...
    const [newProd, setNewProd] = useState({ name: "", price: "", imgUrl: "" });
    const [selectedProductToSimulate, setSelectedProductToSimulate] = useState<string>("");
    ...
    const startScanner = async () => {
      ...
      try {
        ...
        showToast("Cámara de escaneo activada.");
      } catch (err) {
        console.warn("Cámara física no disponible. Se activa el simulador interactivo KFS.");
        showToast("Mocking: Simulador de Escáner QR activado.", "success");
      }
    };
    ...
    const handleSimulatedScan = () => {
      if (!selectedProductToSimulate) {
        showToast("Seleccione un producto para simular.", "error");
        return;
      }
      const prod = db.products.find((p: any) => p.id === selectedProductToSimulate);
      if (prod) {
      ...
      <div className="min-h-screen bg-gray-50 pb-20 font-sans">
        ...
        <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-6 animate-fade-in-up">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <button onClick={startScanner} className="bg-white border border-gray-100 p-8 rounded-[2rem] shadow-sm flex flex-col items-center justify-center gap-4 hover:border-[#C5A184]/50 transition-all cursor-pointer">
              <div className="w-12 h-12 bg-[#0A1128]/5 rounded-full flex items-center justify-center">
        ...
        {showScanner && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex flex-col items-center justify-center p-4 animate-fade-in">
            <div className="w-full max-w-md bg-[#0A1128] border border-white/10 rounded-[2.5rem] p-6 shadow-2xl relative">
              <button onClick={stopScanner} className="absolute right-6 top-6 text-gray-400 hover:text-white cursor-pointer">
                <X size={24} />
              </button>
              ...
              <div className="mt-4 flex justify-center">
                <button onClick={stopScanner} className="px-6 py-2 border border-white/15 text-xs text-gray-300 font-bold rounded-lg cursor-pointer">
                  Cerrar Escáner
                </button>
              </div>
            </div>
  ...
  const MarketplaceView = () => {
    ...
    return (
      <div className="min-h-screen bg-gray-50 pb-20 font-sans">
        <Navbar title="Marketplace Público" />
